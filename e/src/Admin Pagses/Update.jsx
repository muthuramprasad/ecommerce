...
import { updateProductTotals } from "../Reducer/TotalSlice";
...
function AutoLayoutExample() {
  ...
  const handleDeleteProduct = async (productId) => {
    ...
    try {
      await axios.delete(
        `http://localhost:3001/api/delete-product/${productId}`
      );
  ...
  const handleEditProduct = (product) => {
    ...
    setFormData({
      ...
      image2: "",
      image3: "",
      image1Src: product.images[0]
        ? `data:image/jpeg;base64,${product.images[0]}`
        : null,
      image2Src: product.images[1]
        ? `data:image/jpeg;base64,${product.images[1]}`
        : null,
      image3Src: product.images[2]
        ? `data:image/jpeg;base64,${product.images[2]}`
        : null,
    });
    handleOpenModal();
  };
  ...
  // Inside useEffect where you calculate totals
  useEffect(() => {
    const calculateTotals = () => {
      let totalProducts = 0;
      let totalCost = 0;
      let totalMensProducts = 0;
      let totalMensProductsCost = 0;
      let totalWomansProducts = 0;
      let totalWomansProductsCost = 0;
      let totalKidsProducts = 0;
      let totalKidsProductsCost = 0;
      ...
      products.forEach((product) => {
        ...
        switch (product.category.toLowerCase()) {
          case "mens":
            totalMensProducts++;
            totalMensProductsCost += product.price;
            break;
          case "womans":
            totalWomansProducts++;
            totalWomansProductsCost += product.price;
            break;
          case "kids":
            totalKidsProducts++;
            totalKidsProductsCost += product.price;
            break;
          default:
            break;
        }
      ...
      // Dispatch action to update totals in Redux store
      dispatch(
        updateProductTotals({
          totalProducts,
          totalCost,
  ...
  return (
    <div>
      ...
      <Container
        fluid
        style={{ marginTop: "-700px", marginLeft: "30px", padding: "10px" }}
      >
        <Col>
    ...
    <Col>
      ...
      {/* <div>
        <p>Total Product Cost: {TotalProductCost}</p>
        <p>Total Product Cost: {totalProducts}</p>
      </div> */}
    </div>
  );
}
